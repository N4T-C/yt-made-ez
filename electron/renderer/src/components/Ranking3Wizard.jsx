...
import { FaYoutube, FaTimes, FaArrowRight, FaArrowLeft, FaCheckCircle, FaUpload, FaExclamationCircle, FaExternalLinkAlt, FaCheck, FaUndo } from 'react-icons/fa'
...
export default function Ranking3Wizard({ onClose, ytDefaults }) {
    ...
    const startProcessing = async () => {
        ...
        setShareStatus(null); setShareUrl(''); setShareError(''); setVideoFilePath(null)
        try {
            const res = await window.electronAPI.processVideo3({
            ...
            setJobStatus({ status: 'error', progress: 0, message: err.message || 'Failed to start processing' })
        }
    }
    ...
    const handleDownload = async () => {
        ...
        const defaultName = `${videoTitle.trim().replace(/[^a-z0-9]/gi, '_') || 'ranking_video_3'}.mp4`
        try {
    ...
    const handleUpload = async () => {
        ...
        try {
            const res = await window.electronAPI.uploadToYouTube({ 
                jobId, 
                metadata: { ...meta, tags: meta.tags.split(',').map(t => t.trim()).filter(Boolean) }, 
                tokens 
            })
            setYoutubeVideoId(res.videoId); setUploadStatus('success')
            persistRef.current = false; localStorage.removeItem(WIZARD_STATE_KEY)
        } catch (err) { setUploadStatus('error'); setUploadError(err.message) }
    }
    ...
    const handleStartOver = useCallback(() => {
        persistRef.current = false; localStorage.removeItem(WIZARD_STATE_KEY)
        if (jobId) window.electronAPI.cleanupVideos().catch(() => {})
        setStep(0); setDirection(-1); setVideoTitle(''); setLinks(['', '', '']); setCaptions(['', '', '']); setCaptionMode('ai'); setJobId(null); setVideoFilePath(null)
                        ...
                        <motion.div key="s0" custom={direction} variants={stepVariants} initial="enter" animate="center" exit="exit">
                            ...
                            <div className="links-form" style={{ marginBottom: 20 }}>
                                {captions.map((cap, i) => (
                                    <div className="link-input-group" key={i}>
                                        <div className="link-number" style={{ background: `${linkColors[i]}22`, color: linkColors[i], border: `1px solid ${linkColors[i]}44` }}>{i + 1}</div>
                                        <input className="form-input" type="text" placeholder={captionMode === 'manual' ? `Caption for clip ${i + 1}` : 'Auto-generated'} value={cap} onChange={e => updateCaption(i, e.target.value)} onBlur={() => handleBlur(`cap-${i}`, cap)} id={`cap3-${i}`} disabled={captionMode !== 'manual'} />
                    ...
                    {step === 2 && (
                        <motion.div key="s2" custom={direction} variants={stepVariants} initial="enter" animate="center" exit="exit">
                            ...
                            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 24, flexWrap: 'wrap' }}>
                                <button className="btn-secondary" onClick={handleDownload} style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}>⬇️ Save Video</button>
                            ...
                            {shareStatus === 'success' && shareUrl && (
                                <div className="share-link">
                                    ...
                                    <a href={shareUrl} target="_blank" rel="noreferrer" onClick={e => { e.preventDefault(); window.electronAPI.openExternal(shareUrl) }}>{shareUrl}</a>
                                </div>
                            )}
