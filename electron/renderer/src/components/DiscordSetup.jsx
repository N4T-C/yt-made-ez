...
export default function DiscordSetup({ onClose }) {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                // Get initial status
                const { running, logs } = await window.electronAPI.discordStatus()
                setActive(running)
                if (logs) setLogs(logs)

                // Subscribe to push updates
                unsub = window.electronAPI.onDiscordStatus((data) => {
    ...
    const handleToggle = async () => {
        ...
        try {
            if (active) {
                const { status } = await window.electronAPI.discordStop()
                ...
                const { status } = await window.electronAPI.discordStart({ 
                    token: token.trim(), 
                    channelId: channelId.trim(), 
                    filebinKey: filebinKey.trim() || undefined 
                })
                setActive(status.running)
                if (status.logs) setLogs(status.logs)
            }
        } catch (err) {
            setError(err.message || 'Failed to toggle Discord bot')
        }
        setLoading(false)
    }
